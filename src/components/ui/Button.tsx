...
const Button: FC<ButtonHTMLAttributes<HTMLButtonElement> & Props> = ({ isSubmitting, ...props }) => {
  const variants: Record<string, string> = {
    ...
    secondary: "bg-white boder-black text-black border border-slate-400 hover:bg-slate-100",
  ...
  return (
    <button
      {...props}
      className={cx(
        "relative flex h-14 w-full items-center justify-center whitespace-nowrap rounded-md text-base font-bold uppercase transition disabled:bg-opacity-50 disabled:cursor-not-allowed",
        props.className,
