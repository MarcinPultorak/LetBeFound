...
const Questionnaire: FC = () => {
  ...
  const checkIsDisabled = (): boolean => {
    const check = userAnswers.find((item) => item.questionId == currentQuestion.toString());
    return check && check.selectedAnswers && check.selectedAnswers?.length > 0 ? false : true;
  };
